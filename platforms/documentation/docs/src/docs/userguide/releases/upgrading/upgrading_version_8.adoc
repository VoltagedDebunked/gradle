...
The embedded Kotlin has been updated from 1.9.24 to link:https://github.com/JetBrains/kotlin/releases/tag/v2.0.10[Kotlin 2.0.10].
Also see the link:https://github.com/JetBrains/kotlin/releases/tag/v2.0.0[Kotlin 2.0.0] release notes.

The default `kotlin-test` version in JVM test suites has been upgraded to 2.0.10 as well.
...
==== Deprecated certain `@Nested` properties' setters on core Gradle types

Some properties that use `@Nested` annotations contain properties that will be migrated to be `Property`-based in future Gradle versions.
In order for the inner properties to have consistent references that can be used as the source for other lazy APIs, the setters for these `@Nested` properties will be removed in Gradle 9.0.
Instead, the inner properties of the `@Nested` property should be set individually.
