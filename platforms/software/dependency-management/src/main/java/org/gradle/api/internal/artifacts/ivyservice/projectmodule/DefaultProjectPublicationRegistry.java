...
public class DefaultProjectPublicationRegistry implements ProjectPublicationRegistry, HoldsProjectState {
    ...
    private final SetMultimap<Path, PublicationForProject<?>> publicationsByBuildId = LinkedHashMultimap.create();
    ...
    @Override
    public <T extends ProjectPublication> Collection<PublicationForProject<T>> getPublicationsForBuild(Class<T> type, Path buildIdentityPath) {
        synchronized (publicationsByBuildId) {
            Collection<PublicationForProject<?>> buildPublications = publicationsByBuildId.get(buildIdentityPath);
    ...
    public void registerPublication(ProjectIdentity projectIdentity, ProjectPublication publication) {
        ...
        synchronized (publicationsByBuildId) {
            ...
            publicationsByBuildId.put(projectIdentity.getBuildIdentifier().getRawBuildPath(), publicationReference);
        }
    }
